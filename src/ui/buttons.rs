...
fn button_system(
    ...
    mut interaction_query: ParamSet<(
        Query<(&Interaction, &Selectable, &mut UiColor), (Changed<Interaction>, With<Button>)>,
        Query<(&Selectable, &mut UiColor), With<Button>>,
    )>,
) {
    if interaction_query.p0().iter().count() > 0 || selection.is_changed() {
        for (selectable, mut color) in &mut interaction_query.p1() {
        ...
        for (interaction, selectable, mut color) in &mut interaction_query.p0() {
            match *interaction {
                Interaction::Clicked => {
